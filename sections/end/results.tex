...
whereas as the distributions for stimuli from the training and test set were not \cite{STDP_like}.
...
The optimum training set size $n_{train} = 1000$ did not produce remarkably higher misclassifaction rates than bigger training set sizes \cite{STDP_like}.
...
The confusion matrix of \autoref{img:error_analysis} serves as error analysis tool in \cite{SNN}.
