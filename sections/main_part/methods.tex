...
Concepts used include \textcolor{red}{\acfi{LIF} neurons}, \textcolor{red}{lateral inhibition}, \textcolor{red}{intrinsic plasticity} and \textcolor{red}{conductance-based synapses}.

\subsubsection{Learning/ \ac{STDP}}
The synapses of the \ac{SNN} is trained using unsupervised \acfi{STDP}. 
...
\ac{STDP} increases synaptic weight if the postsynaptic neuron reacts immediately after the presynaptic neuron fires, so \cite{object_detection_SNN}.
...
$\eta$ is the learning rate, presynaptic trace $x_{pre}$ tracks the number of recent presynaptic spikes (decaying if no spike arrives, increased by one otherwise), $\mu$ is the dependence of the update on the previous weight, $x_{tar}$ is the target value of the presynaptic trace at the moment of the postsynaptic spike.
If $x_{tar}$ is high and thus, many spikes arrived at the postsynaptic neuron, the weight will possibly not be increased (especially if fewer spikes arrived at the presynaptic neuron indicated by  $x_{pre}$).

\cite{SNN} also outlines other \ac{STDP} learning rules.
Some \ac{STDP} model relies on a teaching signal, which provides the right response. 


\subsubsection{Neuron model}
\label{subsubsec:neuron_model}
In reality, a neuron fires if the membrane's potential crosses the membrane's threshold $\nu_{thresh}$.
After firing the neuron's membrane potential is reset and within the next few milliseconds cannot spike again, so \cite{SNN}.
%
\begin{equation}
    \centering
...
\autoref{eq:membrane_vol_pot} indicates the slope/ change in the membrane voltage over a time constant $\tau$. 
The voltage decays (i.e. first brackets), in influenced by the excitatory synapses adding to the voltage and the inhibitory synapses subtracting from the voltage.

\subsubsection{Synapse model}
\label{subsubsec:synapse_model}
The synapses are the influence of a presynaptic neuron on another neuron.
If a presynaptic spike arrives the weight $w_{i,j}$ between neuron $i$ and neuron $j$ is added to $g_e$.
Otherwise, $g_e$ is decaying.
%
\begin{equation}
    \centering
...
\end{equation}
%
The decay is computed using \autoref{eq:exc_conductance}.
The slope/ change over time constant $\tau$ is a \textcolor{red}{exponential decay}.
...
In order to ensure the neuron have a similar firing rate, the \eN{}'s membrane threshold is calculated by $\nu_{thresh} + \theta$, where $\theta$ is increased every time the neuron fires and exponentially decaying otherwise.
Since the membrane potential is limited to $E_{exc}$ a neuron stops firing while its membrane threshold is higher than the membrane potential.
...
In order to allow all variables to decay between images are a 150ms phase without any input.

\subsubsection{Testing}
First the learning rate is set to zero to fix the neuron's threshold.
After that the training set is presented once more.
The highest response to the ten digit classes is used to assign a class to each neuron, i.e. labels are used.
